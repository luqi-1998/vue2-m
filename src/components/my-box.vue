<template>
  <el-dialog
      ...
      width="50%">
    <div class="box-main">
      <div :style="fatherStyle" class="big-com">
        <vue-drag-resize v-for="(item,index) in comList" :key="index"
                         :h="item.height/2"
                         :minh="30"
                         :minw="30" :parentH="imgObg.height/2" :parentLimitation="true"
                         :parentW="imgObg.width/2" :preventActiveBehavior="false" :w="item.width/2" :x="item.left/2"
                         :y="item.top/2"
                         @clicked="(point)=>{return clickedEvent(point,index)}" @dragging="(point)=>{return draggingEvent(point,index)}" @dragstop="(point)=>{return dragEvent(point,index)}"
                         @resizestop="(point)=>{return resizestopEvent(point,index)}"
                         @resizing="(point)=>{return resizeEvent(point,index)}">
          <div :style="item|filterStyle" class="resize-item">{{ item.title }}</div>
        </vue-drag-resize>
      </div>
      <div class="right-com">
        <div class="add-com">
        ...
        <div class="com-form">
          <el-form :model="form" label-width="80px">
            ...
            <el-row :gutter="20">
              <el-col :span="12">
                <el-form-item label="距顶部:">
                  <el-input v-model="form.top" size="small"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="12">
                <el-form-item label="距左边:">
                  <el-input v-model="form.left" size="small"></el-input>
                </el-form-item>
              </el-col>
            </el-row>
            ...
            <el-form-item label="字体大小:">
              <el-input-number v-model="form.fontSize" :max="50" :min="12" :step="1" controls-position="right"
